          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text('评分:${i.mediaScore['score'].toString()}', style: style),
                Row(
                  children: [
                    ...
                    const Text('·'),
                    const SizedBox(width: 3),
                    Text(Utils.dateFormat(i.pubtime).toString(), style: style),
                  ],
                ),
                Row(
                  children: [
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: () async {
                      ...
                      var res =
                          await SearchHttp.bangumiInfo(seasonId: i.seasonId);
                      SmartDialog.dismiss().then((value) {
                        if (res['status']) {
                          EpisodeItem episode = res['data'].episodes.first;
                          int? epId =
                              res['data'].userStatus?.progress?.lastEpId;
                          if (epId == null) {
                            epId = episode.epId;
                          } else {
